...
if (dbrows($q_addon_cats) == 0) {
	...
	echo "<center>";
	echo $locale['addondb450']."<br />".$locale['addondb451']."<br /><br /><a href='cats.php".$aidlink."'>".$locale['addondb452']."</a>".$locale['addondb453'];
	echo "</center>\n";
	closetable();
  ...
  $result = dbquery("DELETE FROM ".DB_ADDONS." WHERE addon_id='".$_GET['addon_id']."'");
          ...
          notify($addon_submitter_name, $addon_name.$locale['addondb460'].get_addon_status_mail($addon_status_value), $addon_name.$locale['addondb460'].get_addon_status_mail($addon_status_value).$locale['addondb461'].$addon_approved_comment);
			...
			if (is_uploaded_file($_FILES['addon_download']['tmp_name'])) {
				...
				if ($error == "") {
					...
					//chmod($addon_upload_dir.$addon_download_old, 0444); //Found the goblins! :D
					$result = dbquery(
						"UPDATE ".DB_ADDONS." SET 
	...
	if (dbrows($inactive_addons) != 0) {
		...
		while ($addon_data = dbarray($inactive_addons)) {
			echo "<tr class='tbl1'>
			<td class='tbl1' width='*' nowrap><a href='../view.php?addon_id=".$addon_data['addon_id']."' target='_blank' title='".$locale['addondb506']."'>".$addon_data['addon_name']."</a></td>
