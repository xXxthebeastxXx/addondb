...
define("MOD_MAINTENANCE", false);

require_once INFUSIONS."moddb/infusion_db.php";

if (MOD_MAINTENANCE == true && !iADMIN && FUSION_SELF != "maintenance.php") { redirect(INFUSIONS."moddb/maintenance.php"); }

// Screen Shots
$mod_upload_dir_img = INFUSIONS."moddb/img/screenshots/";
$mod_upload_exts_img = array(
	"png" => "image/png",
	"jpg" => "image/jpg",
	"gif" => "image/gif"
);
$mod_upload_maxsize_img = 2000000;
///////////////

// Translations
$trans_upload_dir = INFUSIONS."moddb/files/trans/";
...
// Mod Files
$mod_upload_dir = INFUSIONS."moddb/files/";
$mod_upload_prefix = "submitted_mod_";
$mod_list_dateformat = "%d/%m-%Y";
$mod_upload_exts = array(
	"zip" => "application/zip",
	"rar" => "application/zip",
	"tar" => "application/x-tar",
	"tar.gz" => "application/x-gzip"
);
$mod_upload_maxsize = 2000000;
///////////////


$mod_ratings = array(1 => "Poor", "Average", "Good", "Very Good", "Awesome!");
$mod_types = array(1 => "Infusion", "MOD", "Panel", "Other");
$mod_status = array("Active", "Pending Approval", "Resubmission", "Suspended", "Rejected");
$mod_orderby = array(
	"mod_name" => "MOD Name",
	"mod_author_name" => "MOD Author",
	"mod_date" => "Submission Date"
);
$mod_orderby_dir = array(
	"ASC" => "Ascending",
	"DESC" => "Descending"
);

function get_mod_status($status_id) {
...
function get_mod_status_mail($status_id) {
...
function get_mod_language($l_id) {
...
function get_newest_version_id() {
	$result = dbquery("SELECT version_id FROM ".DB_MOD_VERSIONS." ORDER BY version_h DESC, version_l DESC, version_s DESC LIMIT 1");
	if (dbrows($result)) {
		$data = dbarray($result);
...
function builduserclassoptionlist($min_mod_level, $max_mod_level, $sel_mod_level = 1) {
	$res = "";
	for ($level = $max_mod_level; $level >= $min_mod_level; $level--) {
		$res .= "<option ".($level == $sel_mod_level ? "selected" : "")." value='".$level."'>".getmodlevel($level)."</option>\n";
	}
	return $res;
}
...
function buildversionoptionlist($sel_ver_id = 1) {
	global $db_prefix;
	$res = "";
	$result = dbquery("SELECT * FROM ".DB_MOD_VERSIONS." ORDER BY version_order DESC");
	if (dbrows($result) > 0) {
		while ($data = dbarray($result)) {
      ...
      $result = dbquery(
			...
			$data = dbarray($result);
					...
					if ($msg_settings['pm_inbox'] == "0" || ($data['message_count'] + 1) <= $msg_settings['pm_inbox']) {
						...
						$message_content = str_replace("[SUBJECT]", $subject, $locale['moddb463']);
						...
						if ($send_email == "1") { sendemail($data['user_name'], $data['user_email'], $settings['siteusername'], $settings['siteemail'], $locale['moddb462'], $data['user_name'].$message_content); }
