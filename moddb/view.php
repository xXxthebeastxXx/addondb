...
if (!isnum($mod_id) || dbrows($q_mods) == 0 || ($d_mods['mod_status'] != 0 && !iSUPERADMIN && $d_mods['mod_status'] != 0)) {
  ...
  <link rel="stylesheet" href="lightbox/lightbox.css" type="text/css" media="screen" />
	...
	<td class='tbl1' nowrap><a href='".FUSION_SELF."?download=".$d_mods['mod_id']."' title='".$locale['moddb502']."'>".$locale['moddb415']."</a> ".$download_count."</td>
	</tr>
	</table>
	</td>
	</tr>
	...
	if ($d_mods['mod_approved_user'] != 0) {
		...
		if (($d_mods['mod_valid_xhtml'] == 1) || ($d_mods['mod_valid_css'] == 1)) 
		{ $tdo = "<td class='tbl1' valign='top'><b>".$locale['moddb516'].":</b></td><td align='center' class='tbl1'>"; $tdc = "</td>"; $colspan = "0"; } else { $tdo = ""; $tdc = ""; $colspan = "3"; }
	...
	$result = dbquery("SELECT * FROM ".DB_MODS." WHERE mod_author_name = '".$d_mods['mod_author_name']."' AND (version_id = '8' OR version_id = '12') AND mod_id != '".$d_mods['mod_id']."' AND mod_status = '0' ORDER BY mod_download_count DESC");
	if (dbrows($result) != 0) { 
	...
	$result = dbquery("SELECT * FROM ".DB_MODS." WHERE mod_co_author_name = '".$d_mods['mod_author_name']."' AND (version_id = '8' OR version_id = '12') AND mod_id != '".$d_mods['mod_id']."' AND mod_status = '0' ORDER BY mod_download_count DESC");
	if (dbrows($result) != 0) {
